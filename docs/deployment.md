...
Daymark의 운영 기준은 서울 리전 ECS Express Mode입니다. 애플리케이션, 컨테이너 이미지, 데이터베이스, DNS를 `ap-northeast-2` 기준으로 관리하고, `usedaymark.com`을 운영 도메인으로 사용합니다.
...
운영 문서와 배포 절차는 서울 리전 ECS Express Mode만 기준으로 유지합니다.

## 공개 가능한 운영 위치

아래 값은 공개 문서에 남겨도 됩니다. AWS 계정 ID, RDS 엔드포인트, DB 비밀번호, Google OAuth secret, remember-me secret, webhook URL은 공개 문서에 적지 않습니다.
...
6. Route 53에서 `usedaymark.com`과 `www.usedaymark.com`을 ALB로 연결합니다.
...
GitHub `production` environment에는 아래 값이 필요합니다.

| 변수 | 설명 |
| --- | --- |
| `AWS_ROLE_TO_ASSUME` | GitHub Actions가 assume할 AWS IAM role ARN |
| `ECS_CLUSTER_NAME` | 운영 ECS cluster 이름 |
| `ECS_SERVICE_NAME` | 운영 ECS service 이름 |
...
Daymark 계정은 Google 계정 확인 후 Workspace ID와 비밀번호를 만드는 구조입니다.

Google Cloud Console에서 OAuth 클라이언트를 만들고 아래 redirect URI를 등록합니다.
...
| 이름 | 권장 저장 위치 |
| --- | --- |
| `GOOGLE_CLIENT_ID` | SSM Parameter Store String |
| `GOOGLE_CLIENT_SECRET` | SSM SecureString 또는 Secrets Manager |

Spring Boot 환경 변수 이름으로 넣는 경우:
...
둘 중 한 방식만 일관되게 사용합니다. `production` 프로필에서는 Google OAuth 값이 없으면 애플리케이션 시작을 실패시킵니다.
...
`production` 프로필에서는 Google OAuth, 보안 쿠키, remember-me secret 같은 운영 필수값이 부족하면 애플리케이션이 시작되지 않습니다.
...
- `www.usedaymark.com`: 같은 ALB로 연결
- ALB listener Host header: ECS 기본 URL, `usedaymark.com`, `www.usedaymark.com`
- ALB HTTP listener: `80`으로 들어온 요청을 `HTTPS:443`으로 `HTTP_301` 리디렉션
...
- Google 로그인이 실패하면 OAuth redirect URI, client ID, client secret, HTTPS 도메인을 확인합니다.
- 도메인 오류는 Route 53 record, ALB target, ACM 인증서, ALB `80 -> 443` listener 상태를 확인합니다.
