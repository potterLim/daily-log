...
운영 문서와 배포 절차는 서울 리전 ECS Express Mode만 기준으로 유지합니다. 운영 리소스도 같은 기준으로 정리합니다.
...
`curl -I http://...`는 `301`로 `https://...`를 가리켜야 합니다. 공용 DNS가 ALB IP를 반환하는데 로컬 브라우저만 예전 Namecheap URL Forward로 이동한다면, 해당 기기의 DNS 캐시가 아직 이전 값을 보고 있는 상태입니다.
